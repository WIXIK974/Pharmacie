...
public class VenteDAO {
    ...
    public boolean ajouterVente(Vente vente) {
        ...
        String sqlMiseAJourMedicament = "UPDATE Medicament SET quantite = quantite - ? WHERE numMedicament = ?";
        ...
        try (Connection conn = connector.connectDb()) {
            ...
            try (PreparedStatement pstmtMiseAJour = conn.prepareStatement(sqlMiseAJourMedicament)) {
                pstmtMiseAJour.setInt(1, Integer.parseInt(vente.getQuantiteVendue())); // Assurez-vous que la quantité est un entier
                pstmtMiseAJour.setInt(2, Integer.parseInt(vente.getFk_numMedicament())); // Assurez-vous que fk_numMedicament est un entier

                int rowsAffectedMiseAJour = pstmtMiseAJour.executeUpdate();
                if (rowsAffectedMiseAJour <= 0) {
                    conn.rollback(); // Annulez la transaction si la mise à jour échoue
                    return false;
                }
            }
